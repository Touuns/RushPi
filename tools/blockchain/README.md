...
The validator uses only Node.js built-ins. It parses every blockchain content and schema JSON file, then checks:
...
The reachability solver explores only stopped grid positions with a visited-state set. It proves a geometric path to the exit under the base movement taxonomy. The bounded scenario simulator then executes declared commands, resource costs, UTXO output/fee/change, contracts, revert, and exit objectives. It protects the intended teaching routes without claiming to prove every possible protocol or economic state.
